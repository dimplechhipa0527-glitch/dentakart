...
export const AdminDashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <AdminHeader
        title="DENTAKART ADMIN OPERATIONS"
        subtitle="Real-time B2B marketplace metrics, inventory controls, order pipeline, and doctor analytics"
        actionButton={
          <div className="flex items-center gap-2">
            <Link
              to="/admin/products"
              className="bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold px-3.5 py-2 rounded-xl shadow-md shadow-teal-600/20 transition flex items-center gap-1.5"
            >
              <Plus className="w-4 h-4" /> Add Product
            </Link>
            <Link
              to="/admin/orders"
              className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-200 text-xs font-bold px-3.5 py-2 rounded-xl transition"
            >
              Fulfill Orders ({kpis.pendingOrders})
            </Link>
          </div>
        }
      />
      ...
      <div className="p-6 space-y-6">
        {/* Top 6 KPI Cards Matching Spec Section #9 */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          {/* Pending Orders */}
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-amber-200 dark:border-amber-900/50 bg-amber-50/20 dark:bg-amber-950/20 space-y-1 shadow-xs">
          ...
          {/* Revenue */}
          <div className="bg-gradient-to-br from-teal-50 to-emerald-50 dark:from-teal-950/40 dark:to-emerald-950/40 p-4 rounded-2xl border border-teal-200 dark:border-teal-800 space-y-1 shadow-xs">
            ...
            <p className="text-xl font-black text-teal-800 dark:text-teal-200 truncate">
              ₹{kpis.revenue.toLocaleString('en-IN')}
            </p>
            <span className="text-[10px] text-emerald-600 font-semibold">Net Realized</span>
          </div>

          {/* Low Stock */}
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-rose-200 dark:border-rose-900/50 bg-rose-50/20 dark:bg-rose-950/20 space-y-1 shadow-xs">
            ...
            <Link to="/admin/inventory?status=LOW_STOCK" className="text-[10px] text-rose-600 font-bold hover:underline block">
              Restock Now ➔
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Sales Chart */}
          <div className="lg:col-span-8 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-bold text-sm text-slate-900 dark:text-white">Weekly B2B Clinic Sales Trends</h3>
                <p className="text-xs text-slate-500">Gross revenue generated across all dental product categories</p>
              </div>
              <span className="text-xs font-black text-teal-600 bg-teal-50 dark:bg-teal-950 px-3 py-1 rounded-xl">
                +24.8% vs Last Week
              </span>
            </div>
            ...
            <div className="h-48 flex items-end justify-between gap-3 pt-6 pb-2 border-b border-slate-100 dark:border-slate-800">
              {salesTrends.map((t: any) => {
                ...
                return (
                  <div key={t.period} className="flex-1 flex flex-col items-center gap-2 group">
                    ...
                    <span className="text-xs font-bold text-slate-600 dark:text-slate-400">{t.period}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="grid grid-cols-4 gap-2 pt-2 text-xs">
              <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl">
                ...
                <p className="font-bold text-slate-900 dark:text-white">₹{financials.grossSales.toLocaleString('en-IN')}</p>
              </div>
              <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl">
                <span className="text-slate-400 text-[10px] block font-bold">GST COLLECTED</span>
                <p className="font-bold text-slate-900 dark:text-white">₹{financials.gstCollected.toLocaleString('en-IN')}</p>
              </div>
              <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl">
                <span className="text-slate-400 text-[10px] block font-bold">DISCOUNTS GIVEN</span>
                <p className="font-bold text-emerald-600">- ₹{financials.discountsGiven.toLocaleString('en-IN')}</p>
              </div>
              <div className="p-3 bg-teal-50 dark:bg-teal-950/40 rounded-xl">
                ...
                <p className="font-bold text-teal-700 dark:text-teal-300">₹{financials.netRevenue.toLocaleString('en-IN')}</p>
              </div>
            </div>
          </div>

          {/* Category Share Distribution */}
          <div className="lg:col-span-4 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 space-y-4">
            <h3 className="font-bold text-sm text-slate-900 dark:text-white">Category Sales Volume</h3>
            <p className="text-xs text-slate-500">Distribution of products ordered by specialty</p>

            <div className="space-y-3 pt-2 text-xs">
              <div>
                <div className="flex justify-between font-bold mb-1">
                  <span>Dental Materials (Composite/Bond)</span>
              ...
              <div>
                <div className="flex justify-between font-bold mb-1">
                  <span>Infection Control (Gloves/Bibs)</span>
              ...
              <div>
                <div className="flex justify-between font-bold mb-1">
                  <span>Orthodontics (Brackets/Wires)</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-sm text-slate-900 dark:text-white">Top Spending Doctors (LTV Leaderboard)</h3>
              ...
              {(analytics?.topDoctors || [
                ...
                <div key={doc.id} className="py-3 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="w-6 h-6 rounded-full bg-teal-50 text-teal-700 font-bold flex items-center justify-center text-xs">
                      #{idx + 1}
                    </span>
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white">{doc.name}</p>
                      <span className="text-[11px] text-slate-500">{doc.clinicName} • {doc.orderCount} orders</span>
                    </div>
                  </div>
                ...
                <div key={idx} className="py-3 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[11px] text-slate-500">{prod.brand} • {prod.qty} Units Sold</span>
                    </div>
                  </div>
